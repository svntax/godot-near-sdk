...
var wallet_provider: int = WalletProviders.Wallet.INTEAR
var wallet_url: String setget ,get_wallet_url

var user_config = ConfigFile.new()

func _init(config: Dictionary):
	assert(config.has_all(["network_id", "node_url", "wallet_provider"]))
	
	network_id = config.get("network_id")
	node_url = config.get("node_url")
	wallet_provider = config.get("wallet_provider")
	if wallet_provider == WalletProviders.Wallet.INTEAR:
		wallet_url = "https://wallet.intear.tech"
	elif wallet_provider == WalletProviders.Wallet.MY_NEAR_WALLET:
	...
	if OS.has_feature("JavaScript"):
		if wallet_provider == WalletProviders.Wallet.MY_NEAR_WALLET:
			_handle_my_near_wallet_redirect()

func _handle_my_near_wallet_redirect():
